...
class ApiTestRunner
{
    public function run(array $params): TestRun
    {
        ...
        $env['SELECTED_GROUPS'] = implode(',', $selectedGroups);

        $excludedGroups = array_diff($allGroups, $selectedGroups);
        ...
        if (!empty($selectedGroups)) {
            ...
            if (!empty($excludedGroups)) {
                $cmd[] = '--exclude-group';
                $cmd[] = implode(',', $excludedGroups);
            }
        }

        Log::info('This is the selected group within the cmd', ['cmd' => $cmd]);
